...
function cacheDOMElements() {
  ...
  havesterRebootToolTipElement = document.getElementById(
    "harvesterRebootToolTip",
  );
  wireDroneRebootToolTipElement = document.getElementById(
    "wireDroneRebootToolTip",
  );
  ...
  wireDroneLevelDisplayElement = document.getElementById(
    "wireDroneLevelDisplay",
  );
  harvesterCostDisplayElement = document.getElementById("harvesterCostDisplay");
  harvesterLevelDisplayElement = document.getElementById(
    "harvesterLevelDisplay",
  );
  ...
  factoryLevelDisplaySpaceElement = document.getElementById(
    "factoryLevelDisplaySpace",
  );
...
function displayProjects(project) {
  ...
  projectListTopElement.appendChild(
    project.element,
    projectListTopElement.firstChild,
  );
...
function buttonUpdate() {
  ...
  havesterRebootToolTipElement.innerHTML =
    "+" + spellf(harvesterBill) + " clips";
  wireDroneRebootToolTipElement.innerHTML =
    "+" + spellf(wireDroneBill) + " clips";
...
function updateSwarm() {
  ...
  if (giftCountdown <= 0) {
    ...
    if (milestoneFlag < 15) {
      displayMessage(
        "The swarm has generated a gift of " +
          nextGift +
...
function updatePower() {
  if (humanFlag == 0 && spaceFlag == 0) {
    ...
    powerProductionRateElement.innerHTML = formatWithCommas(
      Math.round(supply * 100),
    );
    powerConsumptionRateElement.innerHTML = formatWithCommas(
      Math.round(demand * 100),
    );
    storedPowerElement.innerHTML = formatWithCommas(Math.round(storedPower));
    facPowConRateElement.innerHTML = formatWithCommas(
      Math.round(fDemand * 100),
    );
    dronePowConRateElement.innerHTML = formatWithCommas(
      Math.round(dDemand * 100),
    );
...
function updateStats() {
  ...
  if (creativityOn) {
    creativityElement.innerHTML = formatWithCommas(creativity);
  }
...
function addProc() {
  if (trust > 0 || swarmGifts > 0) {
    ...
    creativitySpeed =
      Math.log10(processors) * Math.pow(processors, 1.1) + processors - 1;
    ...
    for (var k = strLength; k > 0; k = k - 3) {
      if (k - 3 <= 0) {
        ...
        if (pronounce.toUpperCase() != "zero") {
          ...
          result =
            formatWithCommas(num, 1) + placeValue[unitLookup] + " , " + result;
        }
      }
      unitLookup++;
    }
